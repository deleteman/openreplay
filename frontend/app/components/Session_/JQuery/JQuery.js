...
import JQueryDetails from './JQueryDetails';
...
@connectPlayer((state) => ({
  list: state.jqueryList,
  listNow: state.jqueryListNow,
...
export default class JQuery extends React.PureComponent {
  state = {
    filter: '',
  ...
  onRowClick = (item, index) => {
    ...
    this.setState({ current: item, currentIndex: index, showDetails: true });
    this.props.setTimelinePointer(null);
  };

  closeModal = () => this.setState({ current: null, showDetails: false });
  ...
  render() {
    ...
    console.log("Magical console log: ", this.props)
    const { current, currentIndex, showDetails, filteredList } = this.state;
    // const hasErrors = filteredList.some((r) => r.status >= 400);
    console.log("Rendering jquery inspector...", filteredList)
    return (
      <React.Fragment>
        <SlideModal
          right
          ...
          isDisplayed={current != null && showDetails}
          content={
            current &&
            showDetails && (
              <JQueryDetails
        ...
        <BottomBlock>
          <BottomBlock.Header>
              <span className="font-semibold color-gray-medium mr-4">JQuery</span>
            <div className="flex items-center">
              <Input
